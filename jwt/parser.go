...
func (rec *parser) decrypt(claims *TokenClaims) (err error) {
	...
	if claims.CryptoAlgo == AlgoAES {
		if claims.UserID, err = aes_crypto.DecryptCBC(claims.UserID, rec.conf.CryptoSecret); err != nil {
			return errors.New("encrypted token:aes decrypt failed")
		}
		if claims.Ext, err = aes_crypto.DecryptCBC(claims.Ext, rec.conf.CryptoSecret); err != nil {
			return errors.New("encrypted token:aes decrypt failed")
		}
	}
	if claims.CryptoAlgo == AlgoSM4 {
		if claims.UserID, err = gm_crypto.SM4Decrypt("CBC", rec.conf.CryptoSecret, claims.UserID, false); err != nil {
			return errors.New("encrypted token:sm4 decrypt failed")
		}
		if claims.Ext, err = gm_crypto.SM4Decrypt("CBC", rec.conf.CryptoSecret, claims.Ext, false); err != nil {
			return errors.New("encrypted token:sm4 decrypt failed")
		}
	}
	return
